...
└─────────────────┘     └──────────────────┘     └─────────────┘
                    ...
                    ▼           ▼           ▼           │
              ┌──────────┐ ┌────────┐ ┌─────────┐      │
              │ Claude AI│ │ Stripe │ │  Redis  │      │
              │  Insights│ │  API   │ │ Workers │      │
              └──────────┘ └────────┘ └─────────┘      │
                                                        │
                              ┌─────────────────────────┘
                              ▼
...
- **Structured AI output** — LLM returns JSON with answer + data points + anomalies
- **Exponential backoff** on failed payment retries via Celery workers

## Resume Metrics This Project Demonstrates

- Multi-tenant SaaS architecture serving isolated billing data per organization
